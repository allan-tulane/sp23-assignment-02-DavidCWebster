...
def _subquadratic_multiply(x, y):
  ...
  firstSection = bit_shift(_subquadratic_multiply(xvecLeft, yvecLeft), n)

  # (xL · yR) + (xR · yL) part of the sum
  # since we have to add we call decimal_val
  inner = _subquadratic_multiply(
    xvecLeft, yvecRight).decimal_val + _subquadratic_multiply(
  ...
  thirdSection = _subquadratic_multiply(xvecRight, yvecRight)
